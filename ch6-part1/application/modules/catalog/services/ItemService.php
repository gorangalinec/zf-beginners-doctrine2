...
class Catalog_Service_ItemService implements FormElementOptionsRetrieval {
   ...
   /* Basic query returns StampIem properties as array, for StampItems that are displayable and whose
    * display duration has not expired.
    * input: optional search parameters for grade, price and year.
    * output: StampItem properties are returned as an array. 
    */
   public function runSearchQuery(array $input)
   {
        $qb = $this->em->createQueryBuilder();  
        
        $qb->select(array('s', 'c')) // string 'u' is converted to array internally
            ->from('Square\Entity\StampItem', 's')
            ->innerJoin('s.country', 'c' )
            ->where('s.displaystatus = 1')
            ->andWhere('s.displayuntil >= CURRENT_DATE()');    
                  
        if (array_key_exists('grade', $input)) {
            
            $qb->andWhere('s.grade = :grade');
            $qb->setParameter('grade', $input['grade']);
        } 
        
        if (array_key_exists('year', $input)) {
      
            $qb->andWhere('s.year = :year');
            $qb->setParameter('year', $input['year']);
        }
        
        if (array_key_exists('price', $input)) {
      
            $qb->andWhere(':price BETWEEN s.salepricemin AND s.salepricemax');
            $qb->setParameter('price', $input['price']);
        }
